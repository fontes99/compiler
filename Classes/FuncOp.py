...
class FuncOp(Node):
    ...
    def evaluate(self):
        ...
        if self.value == 'def':
            ...
            if (type(consTable.return_["value"]) != consTable.return_["type"]) : raise ValueError(f"Invalid return {type(consTable.return_['value'])} for return type {consTable.return_['type']}")
            return consTable.return_["value"]